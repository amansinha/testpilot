...
void run_simulator(DualCameraState *s) {
  int err = 0;
  zsock_t *frame_sock = zsock_new_pull(">tcp://172.17.0.1:5559"); //changed...
  ...
  while (!do_exit) {
    ...
    assert(err != -1);
    err = zmq_msg_recv(&frame_msg, frame_sock, 0);
    assert(err != -1);

    assert(zmq_msg_size(&t_msg) >= 8);
    uint8_t* dat = (uint8_t*)zmq_msg_data(&t_msg);

    float t = nanos_since_boot(); // changed
    uint32_t frame += 1;  // changed
