...
def ASdata_sample(plan_sock, path_plan_sock, calibration, poller, cal_status, cal_perc, state, plan, path_plan):
...
def ASstate_control(plan, path_plan, CS, CP, state, events, v_cruise_kph, v_cruise_kph_last, AM, rk,
...
def ASsend_live100_CS(plan, path_plan, CS, VM, state, events, v_cruise_kph, AM, LaC, LoC, angle_offset, v_acc, a_acc, rk, start_time, live100, carstate):
...
def controlsd_thread(gctx=None, rate=100):
  ...
  while True:
    ...
    ASsend_live100_CS(plan, path_plan, CS, VM, state, events, v_cruise_kph, AM, LaC, LoC, angle_offset, v_acc, a_acc, rk, start_time, live100, carstate)
    ...
    # Publish data
    yawrate = VM.yaw_rate(math.radians(path_plan.pathPlan.angleSteers), v_acc)
    beta = (VM.aR+VM.aF*VM.chi - VM.m*v_acc**2/VM.cF/VM.cR/VM.l*(VM.cF*VM.aF-VM.cR*VM.aR*VM.chi))
    beta *= yawrate/(1-VM.chi)/v_acc
    send_array(carla_socket, np.array([v_acc,
                                       path_plan.pathPlan.angleSteers,
                                       -3.3*math.degrees(yawrate),
                                       -3.3*math.degrees(beta)]))
