...
def ui_thread(addr, frame_address):
  ...
  radar_state_sock = sub_sock(context, service_list['live20'].port, addr=addr, conflate=True)
  liveCalibration = sub_sock(context, service_list['liveCalibration'].port, addr=addr, conflate=True)
  controls_state_sock = sub_sock(context, service_list['live100'].port, addr=addr, conflate=True)
  ...
  params_ao, params_ao_average, params_stiffness, params_sr = None, None, None, 1.0
  ...
  while 1:
    ...
    if controls_state is not None:
      v_ego = controls_state.live100.vEgo
      angle_steers = controls_state.live100.angleSteers
      model_bias = controls_state.live100.angleOffset
      curvature = controls_state.live100.curvature
      v_pid = controls_state.live100.vPid
      enabled = controls_state.live100.enabled
      alert_text1 = controls_state.live100.alertText1
      alert_text2 = controls_state.live100.alertText2
      long_control_state = controls_state.live100.longControlState
    ...
    if radar_state is not None:
      d_rel = radar_state.live20.leadOne.dRel + RDR_TO_LDR
      y_rel = radar_state.live20.leadOne.yRel
      lead_status = radar_state.live20.leadOne.status
      d_rel2 = radar_state.live20.leadTwo.dRel + RDR_TO_LDR
      y_rel2 = radar_state.live20.leadTwo.yRel
      lead_status2 = radar_state.live20.leadTwo.status
